...
class Cyc_Trainer():
    ...
    def train(self):
        ...
        for epoch in range(self.config['epoch'], self.config['n_epochs']):
            ...
            # if (epoch % 2 == 0):
            torch.save(self.netG_A2B.state_dict(), f"{self.config['save_root']}axial_epoch{epoch}.pth")
              

    def _3D_inference(self, patient_list, result_path):
        self.netG_A2B.load_state_dict(torch.load(self.config['save_root'] + 'axial_epoch8.pth'))
        ...
        for patient_folder in tqdm(patient_list):
            ...
            if os.path.isdir(patient_path):
                ...
                if os.path.exists(pet_file_path):
                    ...
                    with torch.no_grad():
                        for i in tqdm(range(pet_img.shape[2])):
                            pet_slice = pet_img[:, :, i]
                    ...
                    output_file_path = os.path.join(patient_result_path, 'sagittal_ep15.npy')
